...
bin/tusk commit <task_id> "<message>" <file1> [file2 ...] [--criteria <id> ...]
...
- `bin/tusk-commit.py` — Atomic lint-stage-commit (invoked via `tusk commit`). Runs `tusk lint` (advisory), stages listed files, commits with `[TASK-<id>] <message>` format and Co-Authored-By trailer, then calls `tusk criteria done <id>` for each `--criteria <id>` flag — binding each criterion to the new commit hash atomically.
